#!/usr/bin/env python3
# =============================================================
# MAPA MUNDIAL — Ancestral NCDN variant frequency  v3
# chr1:35,565,741 — Global frequency distribution
# Correcciones v3:
#   - Beringia como polilínea manual por el Ártico real
#   - Colorbar abajo izquierda sin solapamientos
#   - Flechas Out-of-Africa corregidas
#   - xlim ajustado para no perder puntos
# =============================================================
...
populations = [
    ...
    {"name": "Karitiana",      "lat":  -8.5, "lon": -63.5, "freq": 100},
...
# Out-of-Africa → Siberia (arco suave por Arabia/Asia Central)
...
# Out-of-Africa → Europa (arco por el Mediterráneo)
...
# BERINGIA — polilínea manual por el Ártico
# Siberia → Yakutia → Chukotka → Beringia → Alaska → Américas
bx = [84.7, 110.0, 130.0, 148.0, -172.0, -160.0, -145.0, -120.0, -100.0, -75.0, -59.5]
by = [51.4,  58.0,  62.0,  66.0,   66.5,   65.0,   62.0,   55.0,   35.0,   5.0, -11.5]

# Dibujar la línea en dos partes (separadas por discontinuidad en 180°)
# Parte 1: Siberia → Chukotka (hasta 148°E)
idx_split = 4  # índice donde cruza el antimeridiano
ax.plot(bx[:idx_split], by[:idx_split],
        color="#8B1A1A", lw=2.5, zorder=5,
        solid_capstyle="round", solid_joinstyle="round")
# Parte 2: desde -172°W (Beringia) hasta Amazonia — con flecha final
ax.plot(bx[idx_split:-1], by[idx_split:-1],
        color="#8B1A1A", lw=2.5, zorder=5,
        solid_capstyle="round", solid_joinstyle="round")
# Flecha final hacia Surui
ax.annotate("", xy=(-59.5, -11.5), xytext=(-68.0, -3.0),
...
ax.annotate("", xy=(-99.5, 20.0), xytext=(-6.0, 38.0),
...
ax.annotate("", xy=(-59.6, 13.1), xytext=(-6.0, 5.0),
    arrowprops=dict(arrowstyle="-|>", color="#999999", lw=1.2,
                    mutation_scale=8,
...
# --- 6. COLORBAR — abajo izquierda DENTRO del mapa ---------
...
# Muescas de referencia en la colorbar
for val, lbl in [(3, "NFE 3%"), (70, "AFR 70%"), (100, "100%")]:
    frac = val / 100.0
    ax_cb.axvline(frac, color="white", linewidth=0.8, zorder=5)
    ax_cb.text(frac, 1.35, lbl,
...
for fmt, d in [("png", 300), ("pdf", 300)]:
    p = os.path.join(out, f"figure6_world_map_v3.{fmt}")
